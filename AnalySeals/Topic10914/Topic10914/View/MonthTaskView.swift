...
struct MonthTaskView: View
{
    ...
    @Query private var task: [MonthTask]
    ...
    init(date: Binding<Date>, disableAdd: Binding<Bool>)
    {
        ...
        self._task=Query(
            filter: #Predicate<MonthTask> { return $0.date>=start && $0.date<end },
            sort: [SortDescriptor(\MonthTask.date, order: .reverse)],
            animation: .snappy
        )
    }
    ...
    var body: some View
    {
        VStack(alignment: .leading)
        {
            ...
            {index in
                MonthTaskItemView(task: self.task[index])
